...
// Convert non-standard audio (WhatsApp opus/ogg, browser webm, m4a/aac) to WAV
// using the ACE-Step engine's Python venv (PyAV bundles FFmpeg libs).
async function convertToWav(buffer: Buffer, srcExt: string): Promise<Buffer> {
  const os = await import('os');
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'voiceconv-'));
  const srcPath = path.join(tmpDir, `in${srcExt}`);
  const dstPath = path.join(tmpDir, 'out.wav');
  await fs.writeFile(srcPath, buffer);

  const engineBase = process.env.ACESTEP_PATH || path.join('C:', 'LesargeMusicAI', 'ACE-Step-1.5');
  const pyScript = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../scripts/convert_audio.py');

  try {
    await execFileAsync(resolveEnginePython(engineBase), [pyScript, srcPath, dstPath], { timeout: 120000 });
    return await fs.readFile(dstPath);
  } finally {
    fs.rm(tmpDir, { recursive: true, force: true }).catch(() => {});
  }
}

function resolveEnginePython(baseDir: string): string {
  if (process.env.PYTHON_PATH) return process.env.PYTHON_PATH;
  for (const venvDir of ['env', '.venv', 'venv']) {
    const p = path.join(baseDir, venvDir, 'Scripts', 'python.exe');
    try { void fs.access(p); return p; } catch { /* keep looking */ }
  }
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    const allowedTypes = [
      ...
      'audio/aac',
      'video/mp4',
      // WhatsApp voice notes / messenger audio
      'audio/ogg',
      'audio/opus',
      'application/ogg',
      'audio/vnd.wave',
    ...
    if (allowedTypes.includes(file.mimetype) || file.originalname.match(/\.(mp3|wav|flac|m4a|mp4|ogg|oga|opus|webm)$/i)) {
      cb(null, true);
    } else {
      cb(new Error('Invalid file type. Only MP3, WAV, FLAC, M4A, MP4, OGG/OPUS (WhatsApp voice notes), and WEBM are allowed.'));
    }
  }
});
...
router.post('/', authMiddleware, upload.single('audio'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    let ext = path.extname(originalFilename) || '.mp3';
    let mimetype = req.file.mimetype;
    let buffer: Buffer = req.file.buffer;

    // WhatsApp voice notes / browser recordings -> WAV for the engine
    if (/\.(ogg|oga|opus|webm|m4a|aac)$/i.test(ext) || /ogg|opus|webm/i.test(mimetype)) {
      try {
        buffer = await convertToWav(buffer, ext);
        ext = '.wav';
        mimetype = 'audio/wav';
      } catch (convErr) {
        console.warn('Audio conversion failed, storing original file:', convErr);
      }
    }
    ...
    const result = await pool.query(
      `INSERT INTO reference_tracks (user_id, filename, storage_key, file_size_bytes, tags)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [userId, originalFilename, key, buffer.length, tags]
    );
