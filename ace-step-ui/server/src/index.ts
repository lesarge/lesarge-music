...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // In development allow any origin (dev server may be reached via
    // localhost, LAN IPs, or a remote host/IP)
    if (config.nodeEnv === 'development') {
      return callback(null, true);
    }
    // Allow localhost and 127.0.0.1 on any port in production
...
// Serve static audio files (from the configured writable audio dir)
...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'Lesarge Music AI API' });
});
...
app.get('/song/:id', async (req, res) => {
  ...
  try {
    ...
    const description = `🎵 ${song.style} • Create your own AI music free on Lesarge`;
...
<head>
  ...
  <meta property="og:site_name" content="Lesarge">
...
<body>
  <p>Redirecting to <a href="${config.frontendUrl}?song=${song.id}">Lesarge</a>...</p>
</body>
</html>`);
  } catch (error) {
...
if (fs.existsSync(frontendDist)) {
  ...
  console.log('Frontend dist not found - expecting Vite dev server (port 3002).');
}
...
// Start server on all interfaces for LAN access (after DB migrations)
migrate()
  .then(() => {
    app.listen(config.port, '0.0.0.0', () => {
      console.log(`Lesarge Music AI Server running on http://localhost:${config.port}`);
