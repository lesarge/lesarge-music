import { writeFile, mkdir, copyFile, rm, readFile, rename } from 'fs/promises';
import { spawn, execSync } from 'child_process';
import { existsSync } from 'fs';
import path from 'path';
import { handle_file } from '@gradio/client';

async function copyFileWithRetry(src: string, dest: string, attempts = 5): Promise<void> {
  let lastErr: unknown;
...
async function downloadGradioAudioFile(
  ...
  if (fileObj.path && existsSync(fileObj.path)) {
    await copyFileWithRetry(fileObj.path, destPath);
    return;
  }
...
// Auto-select the first available/working model, preferring fast turbo variants
...
async function pickWorkingModel(): Promise<string> {
  try {
    ...
    if (res.ok) {
      ...
      const names: string[] = ((data && data.models) || [])
        .map((m: any) => m.name)
        .filter(Boolean);
...
async function processGeneration(
  ...
  // Auto-select the working model when none was requested
  if (!params.ditModel) {
  ...
  try {
    ...
    console.warn(`Job ${jobId}: model switch failed, continuing with active model`, modelErr);
  }
...
// Generation timeout - configurable; slow CPU machines need well over 10 minutes
const GENERATE_TIMEOUT_MS = parseInt(process.env.LMA_GENERATE_TIMEOUT || '3600000', 10);

function runPythonGeneration(scriptArgs: string[], timeoutMs: number = GENERATE_TIMEOUT_MS): Promise<PythonResult> {
